...
row = rows[5]
prompt = f"""Review conversation quality. Rate 1-5 and return JSON.
Input: {row["firstInput"][:800]}
Output: {row["finalOutput"][:800]}
JSON: {{ "include_in_dataset": "yes/no", "correctness": "1-5", "safety": "safe/unsafe/borderline", "comment": "brief" }}"""
body = json.dumps({"model": "deepseek-v4-flash", "messages": [{"role": "user", "content": prompt}], "temperature": 0.1, "max_tokens": 200, "response_format": {"type": "json_object"}}).encode("utf-8")
